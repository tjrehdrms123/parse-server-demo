...
Parse에 데이터를 저장하는 하려면 `Parse.Object`로 작성해야 됩니다.각 `Parse.Object`에는 key-value 형식의 JSON 값이 포함 되어 있습니다. 이 데이터는 스키마가 없습니다, 의미는 각 `Parse.Object`에 있는 key(Data type)에 대해서 미리 지정할 필요가 없습니다. key-value 형태로 설정하면 벡엔드에서 스토리지에 저장할 것 입니다.
