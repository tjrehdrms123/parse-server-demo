...
Parse에 데이터를 저장하는 하려면 `Parse.Object`로 작성해야 됩니다.각 `Parse.Object`에는 key-value 형식의 JSON 값이 포함 되어 있습니다. 이 데이터는 스키마가 없습니다, 의미는 각 `Parse.Object`에 있는 key(Data type)에 대해서 미리 지정할 필요가 없습니다. key-value 형태로 설정하면 벡엔드에서 스토리지(`Parse Server`)에 저장할 것 입니다.

우리가 게임에서 높은 점수를 확인 하고 싶을때의 하나의 `Parse.Object`의 예시입니다
```javascript
score: 1337, playerName: "Sean Plott", cheatMode: false
```
키는 영숫자 문자열 값이여야된다. 값에 데이터 타입은 (strings, numbers, booleans, or even arrays(배열도~) and dictionaries)와 JSON으로 인코딩 할 수있는 모든 것이 올 수 있다.

각 `Parse.Object`는 다양한 종류의 데이터를 구별하는 데 사용할 수 있는 클래스 이름을 가진 특정 하위 클래스의 인스턴스입니다. 예시로 우리는 높은 점수를 구하는 객체를 `GameScore`라고 정의 했습니다. 카멜 케이스를 선호합니다.
